...
describe('validateValue', () => {
    test('should validate a username correctly', () => {
        ...
        expect(result).toEqual({isValid: true, errorMessage: 'Username must be at least 4 characters.'});

        const result2 = validateValue('username', 'abc');
        expect(result2).toEqual({isValid: false, errorMessage: 'Username must be at least 4 characters.'});
    });

    test('should validate an age correctly', () => {
        const result = validateValue('age', 25);
        expect(result).toEqual({isValid: true, errorMessage: 'Age must be between 1 and 100.'});

        const result2 = validateValue('age', 101);
        expect(result2).toEqual({isValid: false, errorMessage: 'Age must be between 1 and 100.'});
    });
    ...
    test('should validate an email correctly', () => {
        ...
        expect(result).toEqual({isValid: true, errorMessage: 'Invalid email address.'});

        const result2 = validateValue('email', 'not-an-email');
        expect(result2).toEqual({isValid: false, errorMessage: 'Invalid email address.'});
    });
});
